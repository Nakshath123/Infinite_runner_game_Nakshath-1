...
function draw() {
  ...
  if(gameState === play) {
    ...
    fill("white");
    textSize(40);
    text("GAME OVER!",canvas.width/2-400,canvas.height/2+10);
    ...
    text("Refresh the page to play again !",canvas.width/2-400,canvas.height/2+300);
  ...
  if(gameState === instruction) {
    ...
    textSize(50);
    text("GAME-SPACE SHOOTERS",canvas.width/2-300,canvas.height/2-300);
    text("ENJOY THE GAME !",canvas.width/2-300,canvas.height/2+100);
    stroke("yellow");
    fill("yellow");
    textSize(35);
    text("  Some asteroids are coming towards the Earth !",canvas.width/2-300, canvas.height/2 - 210);
    text("  You are a space fighter.",canvas.width/2-300,canvas.height/2-170);
    text("  Press the 'spacebutton' to shoot.",canvas.width/2-300,canvas.height/2-90);
    text("  Use the right and left arrows to move.",canvas.width/2-300,canvas.height/2-50);
    text("  Press 's' to start the game.",canvas.width/2-300,canvas.height/2-10);
